...
func (q *Queries) DeletePastEvents(ctx context.Context) error {
	_, err := q.db.ExecContext(ctx, deletePastEvents)
	return err
}
...
func (q *Queries) ListFriendsAvailableEventIDs(ctx context.Context, arg ListFriendsAvailableEventIDsParams) ([]int64, error) {
	rows, err := q.db.QueryContext(ctx, listFriendsAvailableEventIDs, arg.RequesterID, arg.PlayerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
