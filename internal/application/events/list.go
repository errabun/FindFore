...
func (s *Service) ListForGroup(ctx context.Context, actorID, groupID int64) ([]entity.EventWithDetails, error) {
	...
	result, err := s.events.ListUpcomingByGroupID(ctx, groupID)
	if err != nil {
		return nil, fmt.Errorf("list group events: %w", err)
	}
	if err := s.hydrateRSVPs(ctx, result); err != nil {
		return nil, err
	}
	return result, nil
}
...
func (s *Service) ListJoinableFromGroups(ctx context.Context, actorID int64) ([]entity.EventWithDetails, error) {
	...
	rows, err := s.events.ListJoinableGroupDetails(ctx, actorID)
	if err != nil {
		return nil, fmt.Errorf("list joinable group events: %w", err)
	}
	if err := s.hydrateRSVPs(ctx, rows); err != nil {
		return nil, err
	}

	out := make([]entity.EventWithDetails, 0, len(rows))
	for i := range rows {
		if containsPlayer(rows[i].Accepted, actorID) || rows[i].RemainingSpots <= 0 {
			continue
		}
